...
* so use WaitTools explicit wait methods as much as possible.
...
public class WaitTools 
{
	...
	public static final int DEFAULT_WAIT_4_ELEMENT = 13; 
	...
	public static boolean waitForElementNotPresent(WebDriver driver, final By by, int timeOutInSeconds) {
		boolean isElementPresent = true; 
		try{
			driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS); //nullify implicitlyWait() 
			
			WebDriverWait wait = new WebDriverWait(driver, timeOutInSeconds); 
			isElementPresent = wait.until(ExpectedConditions.not(ExpectedConditions.presenceOfElementLocated(by)));
			System.out.println("isElementPresent: "+ isElementPresent);
			driver.manage().timeouts().implicitlyWait(DEFAULT_WAIT_4_PAGE, TimeUnit.SECONDS); //reset implicitlyWait
			return isElementPresent; //return the element
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static WebElement waitForElementDisplayed (WebDriver driver, WebElement webElement, int timeOutInSeconds) {
	...
	public static boolean waitForElementNotDisplayed (WebDriver driver, WebElement webElement) {
		...
		try{
			...
			int intWaitCounter = 10;
			while(webElement != null || isElementPresent) {
//				System.out.println("in While...");
				try{
				webElement = wait.until(ExpectedConditions.visibilityOf(webElement));
//				System.out.println("Thobber existing..."+ webElement.toString());
				} catch (TimeoutException e) {
//					System.out.println("Thobber Not existing..."+ webElement.);
					isElementPresent = false;
					break;
				}
//				System.out.println("isElementPresent = "+ isElementPresent +" || intWaitCounter = "+ intWaitCounter);
				if(intWaitCounter-- == 0)
					break;
			}
//			System.out.println("isElementPresent = "+ isElementPresent);
...
//			System.out.println("isElementPresent = "+ isElementPresent);
   ...
   /**
	...
	/**
	 * Checks if an Alert box being displayed or not.
	 * @param driver - The driver object to use to perform this element search
	 * @param int - The time in seconds to wait until returning a failure
	 * @return boolean
	 */
	public static boolean isAlertPresent(WebDriver driver, int waitTime_InSeconds) {
		WebDriverWait wait = new WebDriverWait(driver, waitTime_InSeconds);
		try{
	...
	public static WebElement waitForElementisClickable (WebDriver driver, WebElement webElement, int timeOutInSeconds) {
		try{
			driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS); //nullify implicitlyWait() 
			
			WebDriverWait wait = new WebDriverWait(driver, timeOutInSeconds); 
			webElement = wait.until(ExpectedConditions.elementToBeClickable(webElement));
			WaitTools.sleep(timeOutInSeconds);
			driver.manage().timeouts().implicitlyWait(DEFAULT_WAIT_4_PAGE, TimeUnit.SECONDS); //reset implicitlyWait
			return webElement; //return the element
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * Checks if an title contains the mentioned String or not.
	 * @param driver - The driver object to use to perform this element search
	 * @param String - Page Title Content to match with exact match
	 * @return boolean
	 */
	public static boolean isTitlePresent(WebDriver driver, String strTitleContent, int waitTime_InSeconds) {
		WebDriverWait wait = new WebDriverWait(driver, waitTime_InSeconds);
		try{
		    wait.until(ExpectedConditions.titleContains(strTitleContent));
		}
		catch (Exception e){
			e.printStackTrace();
			return false;
